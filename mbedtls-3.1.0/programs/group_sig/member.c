...
int gen_sign( struct pk_struct pk, struct cert_struct cert, struct sign_struct *sign )
{
  ...
  mbedtls_mpi_mod_mpi( &mpi_val, &mpi_val, &pk.n ); // 1 / a^r2 mod
  ...
  mbedtls_mpi_mul_mpi( &d1, &mpi_val1, &mpi_val2 );  // ( 1 / y^r3) ( 1/ a^r2 ) * T1^r1 
  mbedtls_mpi_mod_mpi( &d1, &d1, &pk.n ); // ( 1/ a^r2 ) * ( T1^r1 ) * ( 1 / y^r3 ) mod n 
  ...
  mbedtls_mpi_mod_mpi( &mpi_val, &mpi_val, &pk.n ); // 1 / g^r3  mod n
  mbedtls_mpi_exp_mod( &mpi_val1, &sign->T2, &r1, &pk.n, NULL ); // T2^r1
  mbedtls_mpi_mod_mpi( &mpi_val1, &mpi_val1, &pk.n ); // T2^r1  mod n
