...
mbedtls_sha512_context ctx_hash;

// void add_hash( mbedtls_mpi x )
// {
//   int ret;
//   size_t len = mbedtls_mpi_bitlen( &x ); // get size
//   unsigned char *buf = (unsigned char *) malloc(len); // initlize buffer
//   mbedtls_mpi_write_string( &x, 10, buf, len, len); // mpi to unsigned hash
//   if ( ( ret = mbedtls_sha512_update( ctx_hash, buf, len) ) != 0)
//   {
...
//   mbedtls_sha512_init( &ctx_hash );
//   if ( ( ret = mbedtls_sha512_starts() ) != 0 )
//   {
//     mbedtls_printf( "ERROR. mbedtls_sha512_starts_ret returns %d \n.", ret  );;
//   }
...
//   if( ( ret = mbedtls_sha512_finish( &ctx_hash, c_char ) ) != 0 )
//   {
//     mbedtls_printf( "ok. Finilize hash, please wait...\n" );
//     fflush( stdout );
//   }
//   mbedtls_mpi_read_string( &c, 10, c_char );
